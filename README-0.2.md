# Radar PL 0.2.0 — limity przy fotoradarach i OPP

Zaktualizowana natywna aplikacja Android, Android 8+, bez konta i własnego serwera. Wersja 0.2.0 (versionCode 2) ma ten sam identyfikator i ten sam klucz testowy co 0.1.0. Można instalować ją jako aktualizację, bez odinstalowywania poprzedniej wersji.
...
1. Otwórz APK 0.2.0 na telefonie i zaakceptuj aktualizację. Przy pierwszej instalacji Android może poprosić o zezwolenie na instalację z wybranej aplikacji.
2. Na postoju wybierz Symulacja (około 2 minut). Powinny wystąpić: alarm radaru, limit 50, alarm i wjazd OPP z limitem 70 oraz koniec OPP. To dane demonstracyjne.
...
**Limit dotyczy miejsca pomiaru, niekoniecznie bieżącej pozycji auta.** Nie dodano alarmu przekroczenia limitu na dojeździe: niższy limit może zaczynać się dopiero bliżej kamery. Dla OPP wyświetlamy limit tylko w obsługiwanym kierunku. Nie pokazujemy jeszcze średniej prędkości OPP.
...
java com.sun.tools.javac.Main -d build/test-classes app/src/main/java/pl/radar/offline/Engine.java app/src/main/java/pl/radar/offline/LzString.java tests/EngineTest.java
java -cp build/test-classes pl.radar.offline.EngineTest
```

APK: `build/manual/RadarPL-0.2.0.apk`. Alternatywnie projekt Gradle / Android Studio, AGP 8.7.3, Gradle 8.9. Wersję dostarczoną zbudowano bezpośrednio oficjalnymi narzędziami SDK.

Wykonano 12 testów dopasowania danych w Pythonie i 23 asercje silnika Java. Obejmują m.in. sprzeczne/warunkowe limity, brakujące fragmenty i rozłączną geometrię OPP, kierunek jednokierunkowy, różne limity kierunkowe, utratę GPS, ukrywanie nieznanych limitów oraz scenariusz przejazdu. Testy syntetyczne nie zastępują sprawdzenia oznakowania na drodze. Kompilacja DEX/APK i podpis sprawdzane przy budowaniu. Bez testu urządzeniowego.
