# Radar PL 0.4.0 — automatyczne aktualizacje bez własnego serwera

Zaktualizowana natywna aplikacja Android, Android 8+, bez konta i własnego serwera. Wersja 0.4.0 (versionCode 4) ma ten sam identyfikator i ten sam klucz testowy co 0.1.0, 0.2.0 i 0.3.0. Można instalować ją jako aktualizację, bez odinstalowywania poprzedniej wersji.

## Aktualizacje 0.4

- GitHub Actions pobiera lokalizacje CANARD codziennie, a raz w tygodniu ponownie dopasowuje limity OSM.
- GitHub Pages udostępnia statyczny manifest i dwie paczki gzip; projekt nie wymaga własnego serwera.
- Aplikacja sprawdza manifest po Wi-Fi najwyżej raz na dobę. Po pierwszym pobraniu pobiera tylko komponent, którego SHA-256 zmienił się od poprzedniej wersji.
- Przed zapisem sprawdza HTTPS, schemat, rozmiary, SHA-256, współrzędne, limity, identyfikatory i skalę zmiany liczby urządzeń. Po błędzie zachowuje poprzednią bazę.
- Aktualizacja jest wstrzymywana podczas jazdy i aktywnego OPP. Ręczny przycisk aktualizuje kamery oraz limity.
- Kanał danych: `https://dominikdziadek-ctrl.github.io/radar-pl-data/`. Zacznie działać po jednorazowym opublikowaniu repozytorium i włączeniu GitHub Pages.

Instrukcja publikacji znajduje się w `AUTOMATIC-UPDATES.md`. Klucz podpisujący APK nie może trafić do publicznego repozytorium.
...
1. Otwórz APK 0.4.0 na telefonie i zaakceptuj aktualizację. Przy pierwszej instalacji Android może poprosić o zezwolenie na instalację z wybranej aplikacji.
...
## Aktualizacje bez własnego serwera

Telefon nie łączy się bezpośrednio z Overpass ani nie przetwarza danych OSM. Pobiera wyłącznie statyczne, wcześniej sprawdzone pliki z GitHub Pages. Workflow wykonuje zapytania OSM centralnie raz w tygodniu i zachowuje raport dopasowania. Nowe lub przesunięte kamery bez zgodnego przypisania pozostają bez limitu.

Wbudowane paczki `app/src/main/assets/canard.json` i `osm-limits.json` umożliwiają pracę przed pierwszym połączeniem oraz po awarii kanału aktualizacji.
